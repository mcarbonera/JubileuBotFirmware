...
static void fn_controladorFuzzy(t_sm_ControladorFuzzy *sm) {
    /* Saturar medidas acima de 70 cm para desconsiderar distancias "grandes". */
    #define DISTANCIA_GRANDE 70.0
    int i;
    for(i=1; i<4; i++) {
        if(sm->jubileuInputOutput->DistanciaSensor[i] > DISTANCIA_GRANDE &&
...
uint8_t checkEventProgressMadeFuzzy(t_sm_ControladorFuzzy *sm) {
    if(sm->distanciaObjetivo < (sm->d_prog - D_PROG_EPSILON_FUZZY)) {
        sm->d_prog = fmin(sm->distanciaObjetivo, sm->d_prog);
        return 1;
    //} else if(fabs(sm->distanciaObjetivo - sm->d_prog) <= D_PROG_EPSILON_FUZZY) {
    //    return 1;
    }
...
void calculaVetorAOFuzzy(t_sm_ControladorFuzzy *sm) {
    ...
    // vetor velocidade:
    /*
    if(sm->eoFis.recVel > VEL_P_X) {
        sm->eoFis.recVel = VEL_P_X;
    }
    */
...
void calculaRecomendacaoFinal(t_sm_ControladorFuzzy *sm) {
    int i;
    double alpha = 1.6;
    double coeffAtivacao = fmin(fmax(sm->ativacaoSP, ATIVACAO_MARGEM), ATIVACAO_PASSOS-ATIVACAO_MARGEM) - ATIVACAO_MARGEM;
...
void calculaSaidas(t_sm_ControladorFuzzy *sm) {
    int i;
    double percMinVel = 0; // necessário para o robô parar e adotar comportamento de seguir parede.

    for(i=0; i<2; i++) {
        sm->svFis.recW[i] = JUBILEU_MIN_VEL * percMinVel + sm->svFis.recW[i] * (JUBILEU_MAX_VEL - JUBILEU_MIN_VEL * percMinVel);
    }
