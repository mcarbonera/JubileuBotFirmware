...
void ExecutarSupervisorHibrido(t_sm_ControladorHibrido *sm)
{
    ...
    trocarVetorDistancia(sm->jubileuInputOutput);
...
#ifdef USAR_MEDIA_MOVEL_APENAS_PARA_TRANSICAO
void trocarVetorDistancia(JDInputOutput *jubileu) {
    volatile double pDistancia;
    int i;
    for(i=0; i<5; i++) {
        pDistancia = jubileu->DistanciaSensor[i];
        jubileu->DistanciaSensor[i] = jubileu->DistanciaSensorTransicao[i];
        jubileu->DistanciaSensorTransicao[i] = pDistancia;
    }
}
#endif
