...
static uint32_t c_start, c_stop, c_final;
...
//                                  ^ Sample Sequencer        ^ Priority of Sample Sequencer
    ...
    while(1)
    {
        ...
        //JubileuData.jubileuInputOutput.motorEsquerdo = 0.001*teste;
        //JubileuData.jubileuInputOutput.motorDireito = 0.001*teste;
        ...
        /*
        if(JubileuData.supervisores.dataControladorHibrido.controladorAtual == ST_FOLLOW_WALL) {
            Exec_SM_Comm(&StateMachineComm,EV_INFORMARTRANSICAOCONTROLADOR);
            CoordenadaParaString();
            DistanciasSensoresParaString();
            MontaMensagem(&StateMachineComm, JubileuData.CoordString);
            Exec_SM_Comm(&StateMachineComm,EV_ENVIARMENSAGEM);
        ...
        */
        ...
        if(StateMachineComm.state==ST_RESPONDECLIENTE_MUDACONTROLADOR) {
            if(JubileuData.supervisores.supervisorAtual == ST_SUPERVISOR_HIBRIDO) {
                ...
                if(JubileuData.supervisores.dataControladorFuzzy.controladorAtual == ST_STOP_FUZZY) {
                    MontaMensagemComInicio(JubileuData.ControladorStr, "STOP\r\n",30);
                } else if(JubileuData.supervisores.dataControladorFuzzy.controladorAtual == ST_FUZZY) {
                    MontaMensagemComInicio(JubileuData.ControladorStr, "FUZZY\r\n",30);
                }
            }
...
void calcularDistanciasSensores(int idxInicio, int idxFim)
{
    ...
    for(i=idxInicio; i<idxFim; i++)
    {
        ...
        JubileuData.jubileuInputOutput.DistanciaSensor[i] = JubileuData.jubileuInputOutput.DistanciaSensorMediaMovel[i][JubileuData.jubileuInputOutput.iteracaoMediaMovel];
        JubileuData.jubileuInputOutput.DistanciaSensorTransicao[i] = 0;
        for(j=0; j<JANELA_MEDIA_MOVEL; j++) {
            JubileuData.jubileuInputOutput.DistanciaSensorTransicao[i] +=
